...
def checkChapter(chapter,storyId):
    ...
    if result.find('tick')==-1: result=str(opener.open('http://www.fimfiction.net/ajax/toggle_read.php',urllib.parse.urlencode({'chapter':id}).encode('ascii')).read())
...
try:
        ...
        for curStory in curStories:
            ...
            if curStory[0]==stories[nextToProcess]:
                if curStory[1]>=len(curStory[2]): # Chapters loaded
                    ...
                    print('Reset story '+str(curStory[0])+' ('+str(nextToProcess+1)+'/'+str(len(stories))+')')
                    nextToProcess+=1
    ...
    file.close()
    logfile.close()
except KeyboardInterrupt: failWith('Scripted stopped by interrupt')
finally:
