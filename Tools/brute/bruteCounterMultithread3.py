...
def checkChapter(chapter,storyId):
    global totalWords, curStories, nextToPreload, storiesSize, stories
    words=0
    id = chapter['id']
    result=''
    result=str(opener.open('http://www.fimfiction.net/ajax/toggle_read.php',urllib.parse.urlencode({'chapter':id}).encode('ascii')).read())
    opener.open('http://www.fimfiction.net/ajax/toggle_read.php',urllib.parse.urlencode({'chapter':id}).encode('ascii'))
    ...
    for story in curStories:
        if story[0]==storyId:
            story[1]+=1
            story[3]+=words
            if story[1]>=len(story[2]): # We just checked the last chapter, try to preload a new story
                while nextToPreload<STORIES_SIZE: time.sleep(0.01) # We're still in the init preload loop
                if nextToPreload<storiesSize:
                    nextToPreload+=1 # Update nextToPreload asap to prevent another thread checking the same thing at the same time
                    a = nextToPreload-1 # Use a local var to prevent concurrency problems
                    curStories.append([stories[a],-2,[],0])
                    threading.Thread(None, checkStory, None, (stories[a],)).start()
            break
def checkStory(id):
        ...
        if curStory[0]==id:
            ...
            if story['story']['words']!=0: 
                ...
                for chapter in curStory[2]:
                    while threading.active_count()>MAX_THREADS: time.sleep(0.01) # Don't want to risk a 502 or errors because we're going too fast
                    threading.Thread(None, checkChapter, None, (chapter,id)).start()
            else:
                # Complete empty stories for processing, then preload a new story
                curStory[1]=0
                curStory[2]=[]
                while nextToPreload<STORIES_SIZE: time.sleep(0.01) # We're still in the init preload loop
                if nextToPreload<storiesSize:
